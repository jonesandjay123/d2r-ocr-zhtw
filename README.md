...
**問題：** D2R 地上掉落的物品名稱（繁中）需要自動辨識，決定要不要撿。

**洞察：** 遊戲只有 ~1860 種物品，是封閉集合。不需要 OCR「逐字讀」，只需要「這張圖最像哪個已知物品」。

**解法：** 用 CNN 把每張 crop 變成向量，在 1874 個物品名的向量庫裡找最像的。Top-1 命中率 96.6%，零錯撿，每張 30ms。
